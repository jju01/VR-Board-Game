...
public class LerpManager : MonoBehaviour
{
    ...
    public void FollowUI()
    {
        //  Target 을 쫓아 Lerp 이동
        transform.position
            = Vector3.Lerp(transform.position, target.position + target.transform.forward * fowardFactor + target.transform.up * upFactor + target.transform.right * rightFactor, speed * Time.deltaTime);
