...
// 역할 : Test 모드라면, VR Controller 없이도 기능을 수행한다.

// 역할 : 내 손에 닿은(=일정 범위 안) 물체를 잡거나, Ray가 닿는 범위에 물건을 놓고 싶다.
//  A. 잡다
//   - 잡을 수 있는 범위 -> 마커에 닿는 물체? 
//   - 잡은 물체
//  B. 놓다
//  - 충돌체크를 위한 RigidBody
//  - Ray
...
public class Hand_CJY : MonoBehaviour
{
    //  - LineRenderer
    ...
    private void HandController()
    {
        ...
        else
        {
            ...
            if (Input.GetMouseButtonUp(1) && catchObj != null)
            {
                DropObj();
                // LR 비활성화
                //lr.enabled = false;
    ...
    private void CatchObj()
    {
        ...
        if (hits != null && hits.Length > 0)
        {
            ...
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                // - 현재 녀석의 거리 (손- 부딪힌 물체)
                float currentDis = Vector3.Distance(hand.position, hits[selectedIndex].transform.position);
                // - 다음 녀셕의 거리
                float nextDis = Vector3.Distance(hand.position, hits[i].transform.position);
        ...
        if (selectedIndex != -1)
        {
            ...
            catchObj.transform.parent = hand;
            // +타겟의 위치로 맞춰줌
    ...
    // 물리연산 적용 시 실행되는 함수 (GameObject)
    private void EnablePhysics(GameObject obj)
    {
        // 1. Rigidbody가 붙어있는지 가져온다..
